...
func (p *Point) getDistanceFromCenter() int {
	center := Point{0, 0, 0}
...
func NewWire(rawLine string) *Wire {
	...
	w.rawToSteps()
	w.track = make(map[string]Point)
	return w
}

func (w *Wire) rawToSteps() {
	for _, step := range strings.Split(w.rawLine, ",") {
		dist, err := strconv.Atoi(step[1:])
		if err != nil {
			panic(err)
		}
		w.line = append(w.line, Step{
			dir:  string(step[0]),
			dist: dist,
		})
...
// Go through the commands line and make track
func (w *Wire) Go() {
	for _, step := range w.line {
		switch cmd := step.dir; cmd {
		case "R":
			w.makeStepR(step.dist)
			break
		case "L":
			w.makeStepL(step.dist)
			break
		case "U":
			w.makeStepU(step.dist)
			break
		case "D":
			w.makeStepD(step.dist)
			break
		}
	}
}

func (w *Wire) makeStepR(d int) {
	xx := w.currPosition.x + d
	y := w.currPosition.y

	steps := w.currPosition.steps

	for x := w.currPosition.x; x <= xx; x++ {
		w.track[fmt.Sprintf("%d_%d", x, y)] = Point{x, y, steps}
		steps++
	}
	w.currPosition = Point{xx, y, steps - 1}
}

func (w *Wire) makeStepL(d int) {
	xx := w.currPosition.x - d
	y := w.currPosition.y
...
func (w *Wire) makeStepU(d int) {
	x := w.currPosition.x
	yy := w.currPosition.y + d

	steps := w.currPosition.steps

	for y := w.currPosition.y; y <= yy; y++ {
		w.track[fmt.Sprintf("%d_%d", x, y)] = Point{x, y, steps}
		steps++
	}
	w.currPosition = Point{x, yy, steps - 1}
}

func (w *Wire) makeStepD(d int) {
	x := w.currPosition.x
	yy := w.currPosition.y - d
...
func main() {
	...
	g := 999999999999999 // ;)
	for s, p := range w1.track {
		for s2, p2 := range w2.track {
			if s == s2 && s != "0_0" {
				k := p.steps + p2.steps
				if k < g {
					g = k
